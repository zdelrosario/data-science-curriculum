...
df_train <- df_psaap %>% filter(idx %in% seq(1:20))
df_validate <- df_psaap %>% filter(idx %in% seq(21:36))
```
...
__q4__ Use a combination of EDA and train-validation error to build a model by selecting *principled* predictors for the `formula` argument. Document your findings under *observations* below. Try to build the most accurate model you can!
...
- What interval for `T_norm` would you recommend the design time to plan around?
