...
*Note*: In this challenge I provide a lot of stub-code; you won't have to do too much coding. This is because I want you to *focus on answering the conceptual questions*. The most important question in this challenge is *what sources of uncertainty are you accounting for, and what sources are you not accounting for?*
...
*Strength* is a measure of how much internal stress an object can survive: A higher strength means a stronger material. This challenge's data are very simple: They are (synthetic) observations on aluminum strength. 
...
__q1__ Visualize the strength data with a histogram. Describe the extent to which you can tell what "shape" the data have.
...
- To what extent can you tell what shape the data have?
...
## TODO 2: Extract parameter estimates from df_elasticity_norm and df_poisson_norm
...
__q6__ Complete the code below to construct a bootstrap CI on your POF estimate. Answer the questions under *observations* below.

*Hint 1*: Remember that you need to provide results in the form `tibble(term = "pof", estimate = pof_estimate)` to work with `int_pctl()`.

*Hint 2*: If you're having trouble with this part, take a look at the `pr-bootstrap` chunk at the end of `e-stat09-bootstrap`.

```{r q6-task}
## NOTE: You only need to edit one line below!
df_samples %>% 
  bootstraps(times = 1000) %>%
  mutate(
    estimates = map(
      splits,
## TODO: Uncomment and finish this line!
     # ~ analysis(.x) %>% # Uncomment and write your code here!
    )
  ) %>%
  int_pctl(estimates)
```
