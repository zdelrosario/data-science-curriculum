...
*Hint 1*: You can do this in a single pivot using the `".value"` argument and a `names_pattern` using capture groups `"()"`. Remember that you can use an OR operator `|` in a regex to allow for multiple possibilities in a capture group, for example `"(Estimate|Margin of Error)"`.

*Hint 2*: You may find it helpful to drop all of the `"Annotation of .*"` columns in the dataset; you will not need those.
