---
title: "Statistics: Model Selection and the Test-Validate Framework"
...
__q2__ Copy the code from above to fit a `degree = 19` polynomial to the measurements.

```{r q2-task}
## TASK: Fit a ninth-degree polynomial to df_measurements
# task-begin
fit_d19 <- NA
# task-end
# solution-begin
fit_d19 <-
  df_measurements %>%
  lm(data = ., formula = y_meas ~ poly(x, degree = 19))
# solution-end
...
df_truth %>%
  add_predictions(fit_d19, var = "y_pred") %>%
...
__q3__ Compute the `mse` for the 2nd and 19th-degree polynomial models on `df_measurements`. Which model has the lower error? Which hyperparameter value (polynomial degree) would you choose, based *solely* on these numbers?

*Hint*: We learned how to do this in `e-stat11-models-intro`.

```{r q3-task}
# TASK: Compute the mse for fit_d2 and fit_d19 on df_measurements
# solution-begin
mse(fit_d2, df_measurements)
mse(fit_d19, df_measurements)
# solution-end
```
...
<!-- task-begin -->
- Which model---`fit_d2` or `fit_d19`---has lower error on `df_measurements`?
...
<!-- task-end -->
<!-- solution-begin -->
- `fit_d19` has lower error on `df_measurements`.
- Based *solely* on these results, we would be inclined to choose `degree = 19`.
...
__q5__ Build your own model! Choose which predictors to include by modifying the `formula` argument below. Use the validation data to help guide your choice. Answer the questions below.
