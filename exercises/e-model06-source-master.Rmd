...
*Prerequisites*: e-stat03-descriptive, c08-structures
...
The *cause-source quadrants* organize variability into four distinct categories. Today, we're going to focus on the *source* axis, and limit our attention to *chance causes*.

![Variability quadrants](./images/cause-source-quadrants.png)

- *Cause* is an idea from statistical quality control; a *chance cause* is understood to be random, while an *assignable cause* is thought to be traceable and preventable. We will explore cause further in the next exercise.
- *Source* is an idea from statistical education theory; this concept is explained further below.

## Real vs Induced Source

The idea of *source* can only be understood in the distinction between a *target value* and a *measured value*: The *target value* is the quantity that we are seeking to study, while the *measured value* is a possibly-corrupted version of our target. The key insight is that **variability can occur both in the target value, and in the measured value**.
...
Other physical quantities exhibit both real and induced variability. Since the concept of *source* relies on a target value, the only way we can make progress with this concept is to consider a specific scenario in detail.
...
*The Target*: The strength of each steel component---as-manufactured---will ultimately determine whether each aircraft is safe. As we learned in `c08-structures`, a structure is safe if its applied stress is less than its strength. Therefore, a smaller material strength is a more conservative value for design purposes.
