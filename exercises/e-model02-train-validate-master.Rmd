...
__q2__ Copy the code from above to fit a `degree = 18` polynomial to the measurements.
...
fit_over <-
  df_measurements %>%
  lm(data = ., formula = y_meas ~ poly(x, degree = 18))
# solution-end
...
__q3__ Compute the `mse` for the 2nd and 18th-degree polynomial models on `df_measurements`. Which model has the lower error? Which hyperparameter value (polynomial degree) would you choose, based *solely* on these numbers?
...
- Based *solely* on these results, we would be inclined to choose `degree = 18`.
