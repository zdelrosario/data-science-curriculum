...
[*Branches*](https://zellwk.com/blog/git-branch/) are an important concept in git. Basically, any time you're working in a repo with other people, you should use a branch to organize your edits, rather than committing directly to your master branch. This is important because the master branch---by the social contract of open-source software---*should* be guaranteed to work!
...
To push the current branch and set the remote as upstream, use
    ...
    git push --set-upstream origin dev_addname
```

Where `dev_addname` is the name of the branch I created. If you see this, it's because Git is not willing to make assumptions about *where you want to push*. The `--set-upstream origin dev_addname` tells Git to push to `origin` (which should be set to your forked repository if you cloned it in q2 above), while `dev_addname` tells Git to push the changes to the `dev_addname` branch.
...
Click the **Pull request** to start the process of making a pull request (PR). Add a comment justifying the PR, and click **Create pull request**. Your partner will now be able to see your PR.

__q8__ (*Merging*) Open your public repo GitHub page; you should see that your partner has an open pull request. Your task is to *merge* their
