---
title: "The Bootstrap: Some Recipies"
...
*Purpose*: Confidence intervals are an important tool for assessing our estimates. However,
...
The `rsample` function `int_pctl()` will compute (perecentile) confidence intervals from a bootstrap resampling, but we need to compute our own statistics. Remember the `fitdistr()` function from the previous exercise?
...
__q4__ (Number of replicates) First run this code, and comment on whether the approxiamted coverage probability is close to the nominal `0.95`. Increase the value of `n_boot` and re-run; at what point does the coverage probability approach the desired `0.95`?
...
To finish, I'll present some example code on how you can apply the bootstrap to a more complicted problem. In the previous exercise `e-stat08-fit-dist` we estimated a probability based on a fitted distribution. Now we have the tools to produce a bootstrap-approximated a confidence interval for that probability estimate.
