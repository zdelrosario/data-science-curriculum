...
Notice that the `trans` column of `mpg` has many entries of the form `auto|manual\\([[:alpha:]]\\d\\)`; use `mutate()` with `str_extract()` to create a new column `tmp` with just the code inside the parentheses extracting `[[:alpha:]]\\d`.
